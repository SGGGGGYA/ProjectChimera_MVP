...
public class MainMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 自动创建主菜单背景控制器
        if (FindObjectOfType<MenuBackgroundController>() == null)
        {
            var bgGo = new GameObject("MenuBackgroundController");
            bgGo.AddComponent<MenuBackgroundController>();
        }
    ...
    void SafeLoadScene(string sceneName)
    {
        ...
        {
            SceneTransitionManager.Instance.TransitionTo(() => GameManager.LoadSceneClean(sceneName));
        }
        else
        {
