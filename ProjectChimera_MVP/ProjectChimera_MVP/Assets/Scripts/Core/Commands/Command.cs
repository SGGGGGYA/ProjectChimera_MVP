...
public class CommandContext
{
    ...
    public IBattleContext battleContext;
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        if (isAoE)
        {
            ...
            foreach (var enemy in aliveEnemies)
            {
                ...
                if (!hit)
                {
                    ctx.battleContext.SpawnDamagePopup(enemy.transform.position + Vector3.up * 1.5f, 0, PopupType.Miss);
                ...
                ctx.battleContext.DealDamage(ctx.attacker, enemy, dmg);
            }
        }
        else
        {
            ...
            {
                ctx.battleContext.SpawnDamagePopup(ctx.selectedTarget.transform.position + Vector3.up * 1.5f, 0, PopupType.Miss);
            ...
            ctx.battleContext.DealDamage(ctx.attacker, ctx.selectedTarget, dmg);
...
public class HealCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        ctx.battleContext.SpawnDamagePopup(ctx.selectedTarget.transform.position + Vector3.up * 1.5f, healAmount, PopupType.Heal);
...
public class ShieldCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        ctx.battleContext.SpawnDamagePopup(ctx.selectedTarget.transform.position + Vector3.up * 1.5f, shieldAmount, PopupType.Shield);
...
public class ConsumeResourceCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        if (resourceType == ResourceType.Stress)
        {
            if (amount >= 0)
            {
                ctx.battleContext.SpawnDamagePopup(ctx.selectedTarget.transform.position + Vector3.up * 1.5f, amount, PopupType.Stress);
                StressManager.AddStress(ctx.selectedTarget, amount, StressTag.Combat);
                StressManager.CheckResolve(ctx.selectedTarget, ctx.battleContext);
            }
            else
            {
                ctx.battleContext.SpawnTextPopup(ctx.selectedTarget.transform.position + Vector3.up * 1.5f, $"-{-amount}", new Color(0.6f, 0.2f, 0.8f), 3f);
...
public class ModifyAttributeCommand : Command
{
    ...
    public int duration; // 0 = permanent
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        // 把外部传入的 duration 注入到 modifier（如果 modifier 自己没设）
        var mod = modifier;
        if (duration > 0)
        {
            mod.duration = duration;
            mod.remainingTurns = duration;
        }
        ctx.selectedTarget.modifiers.Add(mod);
        string sign = mod.type == ModifierType.Add ? "+" : "x";
        string durStr = mod.duration > 0 ? $" ({mod.duration}回合)" : " (永久)";
        BattleLog.Add($"[属性修改] {ctx.selectedTarget.unitName} {mod.target} {sign}{mod.value}{durStr}");
...
public class BloodthirstyStrikeCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        ctx.battleContext.DealDamage(attacker, target, finalDmg);
        ...
        attacker.UpdateHPUI();
        ctx.battleContext.SpawnDamagePopup(attacker.transform.position + Vector3.up * 1.5f, actualSelfDmg, PopupType.Damage);
        ...
        attacker.UpdateHPUI();
        if (actualHeal > 0)
            ctx.battleContext.SpawnDamagePopup(attacker.transform.position + Vector3.up * 1.5f, actualHeal, PopupType.Heal);
...
public class MindShockCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        if (target.currentHP > 0 && stressAmount != 0)
        {
            if (stressAmount > 0)
            {
                ctx.battleContext.SpawnDamagePopup(target.transform.position + Vector3.up * 1.5f, stressAmount, PopupType.Stress);
                StressManager.AddStress(target, stressAmount, StressTag.Combat);
                StressManager.CheckResolve(target, ctx.battleContext);
            }
            else
...
public class SelfInflictCommand : Command
{
    ...
    public override CommandResult Execute(CommandContext ctx)
    {
        ...
        if (inflictType == SelfInflictType.HP)
        {
            ...
            ctx.battleContext.SpawnDamagePopup(ctx.attacker.transform.position + Vector3.up * 1.5f, dmg, PopupType.Damage);
        ...
        else if (inflictType == SelfInflictType.Stress)
        {
            if (amount >= 0)
            {
                ctx.battleContext.SpawnDamagePopup(ctx.attacker.transform.position + Vector3.up * 1.5f, amount, PopupType.Stress);
                StressManager.AddStress(ctx.attacker, amount, StressTag.SelfHarm);
                StressManager.CheckResolve(ctx.attacker, ctx.battleContext);
            }
            else
            {
                ctx.battleContext.SpawnTextPopup(ctx.attacker.transform.position + Vector3.up * 1.5f, $"-{-amount}", new Color(0.6f, 0.2f, 0.8f), 3f);
