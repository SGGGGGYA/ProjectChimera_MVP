...
> ⚠️ **勘误说明（2026-05-26）**：本文档是 MVP 启动前的规划，实际开发路径与规划有显著差异。以下列出关键不同：
> 1. **技术栈**：项目实际使用 **纯 C# 脚本** 开发，从未使用 Visual Scripting
> 2. **职业数量**：规划为 2 职业（战士/游侠），实际已实现 **4 职业**（+狂战士/学者）
> 3. **敌人类型**：规划为 2 种，实际已实现 **5 种**（哥布林战士/弓箭手/刺客/精英/狼）
> 4. **技能数量**：规划 6 个，实际已实现 **15+ 个**（含狂战士/学者技能）
> 5. **压力系统**：规划为"仅累积，满 100 强制回城"，实际已实现 **完整 0~200 + 美德/崩溃/心脏病**
> 6. **伤害公式**：规划为 `(base + weapon) * STR_coeff - DEF`，实际使用 `CombatSystem` 管线（含命中/闪避/暴击）
> 7. **经验曲线**：规划线性 100/200/400/800/1600，代码实际使用 `100 * 2^(等级-1)` 公式
> 8. **装备系统**：规划仅武器+防具，实际已实现完整 Equipment + StatMod 系统
> 9. **开发工具**：规划第 0 步安装 Unity/Visual Scripting，实际项目已直接使用 Unity C# 开发
>
> 本文档内容保留以反映原始设计意图。
